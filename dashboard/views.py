...
@login_required
@user_passes_test(es_admin)
def vista_borrar_pedido(request, id_pedido):
    pedido = get_object_or_404(Pedido, id=id_pedido)
    
    if request.method == 'POST':
        pedido.delete()
        messages.success(request, f"El Pedido #{pedido.id} ha sido eliminado correctamente.")
        return redirect('dash_ver_pedidos')
    
    contexto = {
        'objeto': f"Pedido #{pedido.id} de {pedido.usuario.email}",
        'titulo_pagina': 'Borrar Pedido',
        'url_cancelar': 'dash_ver_pedidos'
    }
    return render(request, 'dashboard/borrar_generico.html', contexto)